...
sys.path.insert(0, os.path.join(current_dir, ".."))  # racine
...
def print_banner():
    ...
    banner = r"""
╔══════════════════════════════════════════════════════════════╗
║                SYSTÈME D'ANALYSE BOUSSIÈRE AVEC IA           ║
...
def import_modules():
    ...
    try:
        # Essayer d'importer data_extraction
        from data.data_extraction import StockDataExtractor, get_stock_overview, MacroDataExtractor
    ...
    try:
        # Essayer d'importer stockmodeltrain
    ...
    try:
        # Essayer d'importer stockpredictor
    ...
    try:
        # Essayer d'importer dashboard
        from visualization.dashboard import TradingDashboard, create_comparison_dashboard
    ...
    try:
        # Essayer d'importer actions_sp500
        from screening.actions_sp500 import screen_sp500, get_sp500_symbols
...
def run_screening():
    ...
    try:
        ...
        if results['success']:
            ...
            print("💡 Analysez une action avec: python run.py SYMBOLE --advanced")
        else:
...
def compare_stocks(symbols, period, modules):
    ...
    try:
        ...
        for symbol in symbols:
            ...
            extractor = modules['StockDataExtractor'](symbol)
        ...
        if len(data_dict) > 1 and modules['create_comparison_dashboard']:
...
def analyze_stock(symbol, period, mode, advanced, create_dashboard, modules):
    ...
    try:
        if advanced and modules['StockPredictor']:
        ...
        if create_dashboard and modules['TradingDashboard']:
            ...
            try:
                ...
                dashboard.load_data(
                    overview=overview,
                    technical_data=predictor.data if hasattr(predictor, 'data') else None,
                    predictions={
                        'future_prices': [],
                        'return_1d': 0,
                        'return_30d': 0,
...
def main():
    ...
    # Mode screening
    if args.mode == "screen":
        if modules['screen_sp500'] is None:
